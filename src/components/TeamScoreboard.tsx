...
export function TeamScoreboard({ scores, totalPoints = 216 }: TeamScoreboardProps) {
  ...
  const barBackground = leader === 'team1'
    ? 'bg-gradient-to-r from-blue-100 via-blue-50 to-gray-200'
    : leader === 'team2'
    ? 'bg-gradient-to-l from-red-100 via-red-50 to-gray-200'
    : 'bg-gray-200'
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-4 md:p-8 mb-6 md:mb-8 golf-card">
      <div className="grid grid-cols-3 gap-2 md:gap-8 items-center">
        ...
        <div className="text-center flex flex-col items-center">
          ...
          />
          <div className="text-xs md:text-2xl font-black text-gray-900 tracking-tight">CABIN CUP</div>
        </div>
      ...
      <div className="mt-4 md:mt-10 pt-4 md:pt-8 border-t border-gray-200">
        ...
        <div className={`relative w-full rounded-xl h-8 md:h-16 shadow-inner overflow-hidden ${barBackground}`}>
          {/* Blue fill from left */}
          <div
            className="absolute left-0 top-0 bottom-0 bg-gradient-to-b from-blue-500 to-blue-700 rounded-l-xl transition-all duration-500"
            style={{ width: `${team1Percentage}%` }}
          ></div>
          {/* Red fill from right */}
          <div
            className="absolute right-0 top-0 bottom-0 bg-gradient-to-b from-red-500 to-red-700 rounded-r-xl transition-all duration-500"
            style={{ width: `${team2Percentage}%` }}
          ></div>
          {/* Gold win line at center */}
